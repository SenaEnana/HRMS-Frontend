...
const EmployeeDashboard = () => {
  ...
  async function fetchLeaveBalance() {
    try {
      ...
      const url = `http://localhost:5100/api/Leave/GetMyLeaveBalance?userId=${userId}`;
  ...
  async function fetchMyCompliant() {
    try {
      ...
      const userId = getUserIdFromToken(token);
      const url = `http://localhost:5100/api/Complaint/MyCompliants?userId=${userId}`;
      const response = await fetch(url);
  ...
  async function fetchLeaveRequests() {
    try {
      ...
      const userId = getUserIdFromToken(token);
      const url = `http://localhost:5100/api/Leave/MyLeaveRequests?userId=${userId}`;
      const response = await fetch(url);
